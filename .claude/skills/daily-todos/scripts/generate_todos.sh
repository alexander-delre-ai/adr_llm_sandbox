...
if [ "$MARKDOWN_ONLY" = false ]; then
    ...
    TICKTICK_SCRIPT="$PROJECT_ROOT/.claude/skills/ticktick-sync/scripts/sync_todos.py"
    ...
    if [ -f "$TICKTICK_SCRIPT" ]; then
        ...
        if [ $SYNC_EXIT_CODE -eq 0 ]; then
            ...
            echo "💡 Run setup: python3 $PROJECT_ROOT/.claude/skills/ticktick-sync/scripts/auth_setup.py"
            echo ""
