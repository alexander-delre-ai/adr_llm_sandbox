...
- If a workspace path is provided, read `analysis.md` and `transcript.md`/`summary.md` for background
...
**Applied Confluence** (MCP: `user-atlassian-mcp-applied`, cloudId: `6461690f-d275-4167-8055-cc3dc06e03f2`):
- Preferred: `searchAtlassian` with natural language query (uses Rovo Search)
...
**KATA Confluence** (MCP: `user-atlassian-mcp-kata`, cloudId: `eadd00c6-0d3f-4c89-99e3-ad95a0daaa51`):
...
**Applied JIRA (AVP)** (MCP: `user-atlassian-mcp-applied`, cloudId: `6461690f-d275-4167-8055-cc3dc06e03f2`):
```
toolName: searchJiraIssuesUsingJql
jql: "text ~ \"term1\" AND text ~ \"term2\" ORDER BY created DESC"
maxResults: 10
fields: ["summary", "description", "status", "priority", "assignee"]
responseContentFormat: "markdown"
```

**KATA JIRA** (MCP: `user-atlassian-mcp-kata`, cloudId: `eadd00c6-0d3f-4c89-99e3-ad95a0daaa51`):
- Same approach on the KATA project

### Step 5 -- Search codebase (conditional)

**Codebase location**: `/home/alexanderdelre/applied/core-stack`
...
- `SemanticSearch` with `target_directories: ["/home/alexanderdelre/applied/core-stack"]` for broad questions
- `Grep` with `path: "/home/alexanderdelre/applied/core-stack"` for specific symbol/function lookups
- `Glob` with `target_directory: "/home/alexanderdelre/applied/core-stack"` for finding files by pattern
...
- For each finding: short summary, direct link/permalink, date, and relevance note
...
### Slack
- **<channel/DM name>** (<date>): <summary> [link]
...
- **Applied first**: Always search Applied/AVP spaces before KATA spaces in Confluence and JIRA
...
- **Large results**: If a Confluence page is relevant, fetch full content with `getConfluencePage` rather than relying on search excerpts
